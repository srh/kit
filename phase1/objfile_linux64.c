...
void linux64_section_headers(uint32_t prev_end_offset,
  ...
  uint32_t rel_size = uint32_mul(s->relocs.count, sizeof(struct elf64_Rela));
...
void linux64_flatten(struct identmap *im, struct objfile *f, struct databuf **out) {
  ...
  linux64_append_relocations(sti_map, f->symbol_table.count, d, &f->data);
  ...
  linux64_append_relocations(sti_map, f->symbol_table.count, d, &f->rdata);
  ...
  linux64_append_relocations(sti_map, f->symbol_table.count, d, &f->text);
