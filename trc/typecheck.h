...
#define METERR(cs, loc, fmt, ...) ERR("At %.*s:%"PRIz":%"PRIz": " fmt, IM_P((cs)->im, checkstate_g_o_import_name((cs), (loc).pos_start.global_offset)), (loc).pos_start.line, (loc).pos_start.column, __VA_ARGS__)
